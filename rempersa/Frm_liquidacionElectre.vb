...
Public Class Frm_liquidacionElectre
    ...
    Private Sub Cmb_remates_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles Cmb_remates.SelectedIndexChanged
        ...
        Try
            If Not (IsNothing(Cmb_remates.SelectedValue.ToString)) Then
    ...
    Private Sub Button1_Click(sender As System.Object, e As System.EventArgs) Handles Button1.Click
        ...
        End Try
        Dim fechali As Date = Mth_calendario.TodayDate
        factur.FechaFacturacion = fechali.Day & "               " & mesliteral(fechali.Month) & "                                             " & fechali.Year.ToString().Substring(2, 2)
        ...
        Dim liqui_elect As New dte_liquidacion
        ...
        '******************************************************************
        '******************************************************************
        '********* Rutina generadora de liquidaciones electronicas ********
        '*********                    31-01-2017                   ********
        '******************************************************************
        '******************************************************************

        'Dim liqui_elect As New dte_liquidacion
        'Dim nros As Integer = (lottes.Count \ 20) + 1
        Dim cc As New Ccalculaliquidacion
        If String.IsNullOrEmpty(Txt_porcentaje.Text) And porcentaje <> 0 Then
            Txt_porcentaje.Text = 10
            porcentaje = CDec(Txt_porcentaje.Text)
        Else
            porcentaje = CDec(Txt_porcentaje.Text)
        End If
        cc.recibe_datos(factur, lottes, CInt(Txt_nrofactura.Text), nros, mandant.id_mandante, porcentaje, Me.Chk_afecto.Checked, Me.Mth_calendario.SelectionRange.Start)


        Dim lotes_liquielec(nros) As List(Of lotesliquidacionview)
    ...
    Private Sub Txt_porcentaje_KeyPress(sender As System.Object, e As System.Windows.Forms.KeyPressEventArgs) Handles Txt_porcentaje.KeyPress
        If Not IsNumeric(e.KeyChar) And Not (AscW(e.KeyChar) = 8) And Not (AscW(e.KeyChar) = 45) And Not (AscW(e.KeyChar) = 75) And Not (AscW(e.KeyChar) = 107) Then
            e.Handled = True
        End If
