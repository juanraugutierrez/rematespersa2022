Public Class Ccalculaliquidacion
    ...
    Sub calculacarga(ppp As List(Of detalle_liquidacion), porcentaje As Decimal, afe As Boolean, idmandante As Integer, nroliquidacion As Integer, fecha As DateTime, detalle As Boolean)
        ...
        Dim datotipoR As String = InputBox("ingresa dato 2da Linea")
        Dim datoTipoR1 As String = InputBox("ingresa dato 1era linea")
        ...
        l = (From li In contex.liquidacion Join de In contex.detalle_liquidacion On li.id_liquidacion Equals de.id_liquidacion Join lotesl In contex.lotes On de.id_lote Equals lotesl.id_lote Where li.nro_liquidacion = nroliquidacion And li.id_liquidacion = id_Factu Order By CInt(de.nro_lote) Ascending, de.sub_lote Ascending
             Select New lotesliquidacionview With {.idlote = de.id_lote, .idremate = 0, .nrolot = de.nro_lote,
                                                   ...
                                                   .Total = de.precio_final, .mandante = de.mandante,
                                                   .sucursal = de.sucursal,
                                                   .observacion = lotesl.observaciones,
                                                   .fecha = li.fecha,
                                                   .afecto = de.afecto}).ToList()

        'l = Nothing
        'l = (From lote In contex.lotes Where lo.nro_liquidacion = nroliquidacion Order By CInt(lo.nro_lote) Ascending, lo.sub_lote Ascending
        '     Select New lotesliquidacionview With {.idlote = lote.id_lote, .idremate = lote.id_remate, .nrolot = CStr(lote.nro_lote),
        '                                                    .nrosblote = CStr(lote.sub_lote), .descripcion = CStr(lote.descripcion),
        '                                                    .unidades = CDbl(lote.nro_unidades_final), .Punitario = CDbl(lote.precio_unitario_final),
        '                                                    .Total = CDbl(lote.precio_final), .mandante = CStr(lote.mandante), .sucursal = CStr(lote.sucursal),
        '                                                    .observacion = CStr(lote.observaciones),
        '                                                    .fecha = DateTime.Now(),
        '                                                    .afecto = lote.afecto}).ToList()
